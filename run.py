...
parser.add_argument('--use_POS', action='store_true', default=True, help='use POS tags to construct syntactic hyperedges')
...
def main():
    doc_content_list, doc_train_list, doc_test_list, vocab_dic, labels_dic, max_num_sentence, keywords_dic, class_weights, num_pos_hyperedges = read_file(args.dataset, args.use_LDA, args.use_POS)
    ...
    train_data = Data(train_data, max_num_sentence, keywords_dic, num_categories, args.use_LDA, num_pos_hyperedges)
    valid_data = Data(valid_data, max_num_sentence, keywords_dic, num_categories, args.use_LDA, num_pos_hyperedges)
    test_data = Data(test_data, max_num_sentence, keywords_dic, num_categories, args.use_LDA, num_pos_hyperedges)
    
    if args.dataset in ['mr', 'Tweet', 'Pascal_Flickr']:
        model = trans_to_cuda(DocumentGraph(args, pre_trained_weight, class_weights, 
                            len(vocab_dic)+2, len(labels_dic), vocab_dic, num_pos_hyperedges))
    else:
